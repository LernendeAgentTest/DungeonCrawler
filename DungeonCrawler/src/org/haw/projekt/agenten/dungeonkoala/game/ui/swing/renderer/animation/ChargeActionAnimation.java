...
public class ChargeActionAnimation extends AbstractAnimation<Double> {
	private static final String CHARGE_ICON = "img/actions/charge.png";
	...
	public ChargeActionAnimation(int x, int y) {
		super(x, y);
		try {
			_image = ImageIO.read(getClass().getClassLoader().getResource(CHARGE_ICON));
		} catch (IOException e) {
			throw new RuntimeException("Das Bild für die ChargeActionAnimation konnte nicht geladen werden!", e);
		}
	}
