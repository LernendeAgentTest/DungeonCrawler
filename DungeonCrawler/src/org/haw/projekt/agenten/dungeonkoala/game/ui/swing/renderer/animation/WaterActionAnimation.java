...
public class WaterActionAnimation extends AbstractAnimation<Double> {
	private static final String SWITCH_TO_WATER_ICON = "img/actions/SwitchToWater.png";
	...
	public WaterActionAnimation(int x, int y) {
		super(x, y);
		try {
			_image = ImageIO.read(getClass().getClassLoader().getResource(SWITCH_TO_WATER_ICON));
		} catch (IOException e) {
			throw new RuntimeException("Das Bild für die WaterActionAnimation konnte nicht geladen werden!", e);
		}
	}
